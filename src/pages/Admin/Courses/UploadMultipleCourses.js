...
import { createCoursesThroughCsvFile } from "../../../ApiCalls/courseApiCalls";
...
const UploadMultipleCourses = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await createCoursesThroughCsvFile(formData);
      if (response.success) {
        toast.success(
          `Successfully uploaded ${response.data?.length || "multiple"} courses`
        );
        clearFile();
      } else {
    ...
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="max-w-xl mx-auto">
        ...
        <p className="mt-4 text-sm text-gray-500 text-center">
          Make sure your Excel/CSV file follows the required format with
          columns: title, description, level, language, duration, price, author,
          category, type, thumbnailUrl.
        </p>
      </div>
    </div>
  );
